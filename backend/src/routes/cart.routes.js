...
const computeTotals = async (items, promoCode) => {
    ...
    if (promoCode) {
        ...
        if (promo) {
            if (promo.discountType === "percent") {
                ...
                discount = Number(Math.min(subtotal, promo.discountValue).toFixed(2));
            }
        }
    }
...
router.get("/", async (req, res, next) => {
    try {
        ...
        const totals = await computeTotals(items, cart.promoCode);
